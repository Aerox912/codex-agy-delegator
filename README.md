...
| Claude Code | `--strict-mcp-config --mcp-config {"mcpServers":{}} --no-session-persistence --permission-mode acceptEdits` |
...
Submit this to `delegate_to_claude` or `delegate_to_agy`. Runs are asynchronous
unless `waitForCompletion` is true. Review the returned run with
`get_agent_run_report`, then call `apply_agent_run` with `confirm: true`.
